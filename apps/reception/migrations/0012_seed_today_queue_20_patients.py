# Seed today's queue with 20 patients. If the queue already has entries, adds only as many
# as needed to reach 20. Uses new patient IDs (DTL-2026-0021 .. DTL-2026-0040).
# Forward: get_or_create queue for today; ensure 20 queue entries (add patients + entries if needed).
...
def seed_today_queue_20_patients(apps, schema_editor):
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("reception", "0011_seed_today_patients_with_appointment_times_v2"),
        ("users", "0001_initial"),
    ]

    operations = [
        migrations.RunPython(seed_today_queue_20_patients, noop_reverse),
    ]
