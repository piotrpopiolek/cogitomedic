...
### Eksport do PDF (rozdziały 00–05 + ścieżka papierowa)

Tak — można zbudować **jeden plik PDF** z plików `00-przeglad.md` … `05-pacjent-wyniki.md` oraz `04-administrator-paper-intake.md` i `paper_intake_flow.md` (bez checklisty zrzutów).
...
Skrypt składa **rozdziały 00–05** oraz **uzupełnienia o ścieżkę papierową** w jednym pliku PDF. Zamienia ścieżki obrazów z formy `/docs/manual/assets/...` na względne wobec korzenia projektu, żeby Pandoc poprawnie wstawił PNG.
...
| [03-doktor.md](03-doktor.md) | Lekarz w panelu medycznym (`DOCTOR`); `ADMIN` / `MANAGER` — pełen widok kolejki w HTML; sekcja o ścieżce papierowej |
| [04-administrator.md](04-administrator.md) | Administrator systemu (`ADMIN`); w sekcji 11 również konto kierownicze (`Manager`); §3a — skrót `/admin/paper-intake/` |
| [04-administrator-paper-intake.md](04-administrator-paper-intake.md) | **Admin / Manager** — procedura T1 / T1′, warunki, auto-revoke |
| [paper_intake_flow.md](paper_intake_flow.md) | **Wszyscy** — diagram Mermaid T1 / T2 vs ścieżka cyfrowa |
...
**Wersja treści:** 2026-05-03 (dopasuj datę przy aktualizacji UI).
