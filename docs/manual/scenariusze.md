...
| **Objaw** | Księgowość potrzebuje **listy pacjentów** w danym tygodniu **wg daty badania** — albo po **pierwszej publikacji Befundu** (v1), albo po **stawieniu się / złożonej ankiecie** (v2, bez no-show). |
| **Przyczyna (techniczna)** | Raport jest modułem admina — nie ma go w panelu recepcji ani lekarza. Zakres dat filtruje po `queue_date`. Parametr `report_mode=published|attended`. |
| **Co zrobić dziś (instrukcja)** | 1) Zaloguj się na konto z rolą **Accounting**, **Manager** lub **Admin** (np. `https://…/admin/`). 2) W menu po lewej: **Księgowość** → **Raport tygodniowy** (`/admin/accounting/report/`). 3) Wybierz **Wariant raportu**: **v1 — opublikowane Befundy** albo **v2 — stawili się**. 4) **Zakres dat:** domyślnie bieżący tydzień (pon.–niedz.) — dotyczy **dnia badania**; opcjonalnie ustaw **Data od** / **Data do** — tabela odświeży się **automatycznie** po wyborze daty lub wariantu (przycisk „Pokaż raport” jest opcjonalny). 5) Sprawdź podgląd tabeli i sekcję per lekarz. 6) Pobierz plik: **Eksport CSV** lub **Eksport XLSX** — zawiera **wszystkie** wiersze z zakresu (nie tylko bieżącą stronę podglądu). Nazwa pliku zawiera wariant: `accounting_report_{published|attended}_{data_od}_{data_do}.…`. |
