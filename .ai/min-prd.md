...
3. **Lekarz:** formularz medyczny (Befund) – grupy zmian (numery z Wideodermatoskopu), cechy dermatoskopowe, ocena, ryzyko; tekst **generowany z checkboxów, ale edytowalny** („baza, nie klatka”). Szkic vs Opublikowany; publikacja idempotentna (sprawdzenie „publikacja w toku” / `publish_request_id`). **Blokada edycji szkicu:** jeden aktywny edytor na dokument (max 24h), widoczność w kolejce (wiersz / komunikat), zwolnienie przy publikacji lub zamknięciu karty.
4. **Archiwizacja:** Outbox → generowanie PDF → upload HiDrive (mock F1–2, API F3) → SMS (SMSApi). Zdalne ścieżki m.in. `/patients/{uuid}/Befund_v{N}.pdf`, PDF z labu z `/incoming/`, archiwum w `/processed/` (szczegóły w `docs/manual/hidrive_incoming_reception.md`). Retencja: usuwanie PDF z serwera po 30 dniach **tylko gdy** `hidrive_sent` i `sms_sent` true.
