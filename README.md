...
- **Admin / Manager (paper path):** Authorize or revoke the paper-intake path for a queue entry from **`/admin/paper-intake/`** (Unfold HTML under `/admin/`) or via **`POST`/`DELETE`** `/api/v1/queue-entries/<uuid>/paper-intake-authorization` — both are **not** clinic-site scoped for these roles (oversight). Other queue-related API routes may still return **`403`** with `other.api.queue_entry_not_in_scope` when the queue entry is outside the caller's assigned clinics.
...
- **Backend:** Asynchronous pipeline (`GENERATE_PDF` -> `HIDRIVE_UPLOAD` -> `SMS_SEND`) processed through Django 6 Tasks (`django.tasks`) + Transactional Outbox, HiDrive API archiving (OAuth2 refresh token; mock switchable), SMS (logistic-only: template `other.sms.patient_results` with public portal URL `{url}` — no token/deep link to PDF) via SMSApi, 30-day retention policy for local PDFs
...
| `make mutmut-smoke` | **CI-aligned mutmut smoke:** `mutmut_pytest.ini` + mutant generation for `name_normalize` (same as CI job) |
...
- SMS notifications (logistic text only — may include public portal URL; patient retrieves PDF via the results portal with phone+DOB and OTP, not via a tokenized SMS link)
