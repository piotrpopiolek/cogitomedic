...
todos:
  ...
  - id: manager-authorize-ui
    content: Dedykowany widok admina/managera (NIE generyczny Django admin) z akcją „Autoryzuj ścieżkę papierową” — widoczny tylko po WAITING + `appointment_time` + `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h (`apps/medical/constants.py`) + brak intake SUBMITTED + brak dokumentu + brak aktywnej autoryzacji; pole `reason` z formularza (10–500 znaków); osobny przycisk „Cofnij autoryzację” gdy autoryzacja istnieje i dokument nie powstał; widok admina `PaperIntakeAuthorizationAdmin` jako readonly
    status: completed
...
- `other.domain.paper_intake_authorization_too_early` → komunikat z placeholderem `{hours}` (wartość = `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT`; w DB/tłumaczeniach bez sztywnego literału „3 godziny”).
...
- Walidacje domenowe:
  ...
  4. **Zabezpieczenie czasowe:** `now() >= entry.appointment_time + timedelta(hours=PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT)` (ta sama stała co §2.1). Powtarzane dla obrony in-depth, choć autoryzacja już to sprawdziła w T1.
...
- **Mapowanie `DomainError` → HTTP** (wszystkie klucze z prefiksem `other.domain.`* w `other_domain.json`):
  ...
  - `other.domain.paper_intake_authorization_too_early` → 409 Conflict z polem `available_at` w response (`appointment_time` + `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h) — pomocne dla UI managera.
...
- Walidacje czasu: autoryzacja przed `appointment_time` + `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h → 409 z polem `available_at`.
...
> **Stan implementacji (2026-05):** T1 w panelu staff jest pod **`/admin/paper-intake/`** (szablony Unfold): **hub** z wyborem wpisu kolejki (`WAITING`, ostatnie **30 dni** `daily_queue.queue_date`, lista **bez** filtrowania placówki) oraz **strona wpisu** z autoryzacją / cofnięciem. Hub, widok HTML wpisu oraz **REST** `POST`/`DELETE` `/api/v1/queue-entries/<uuid>/paper-intake-authorization` **nie** stosują `get_scoped_clinic_site_ids` dla ADMIN/MANAGER (nadzór); inne endpointy kolejki mogą nadal zwracać `other.api.queue_entry_not_in_scope`. Reguły read-only UI: `apps/medical/paper_intake_policy.py`; stałe `PAPER_INTAKE_*` obok innych stałych medycznych: `apps/medical/constants.py`; kontrola ról HTML: `apps/core/staff_custom_admin.py`. Poniższy §6.1 zawiera pierwotną wizję URL (`/admin-panel/...`); w kodzie użyto ścieżki pod `/admin/`.
...
- **Warunki widoczności przycisku „Autoryzuj”** (egzekwowane w UI; serwis i tak waliduje):
  ...
  - `now() >= entry.appointment_time + timedelta(hours=PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT)` (jeśli niespełnione — UI pokazuje przycisk **disabled** z tooltipem „Dostępne od HH:MM” wg `earliest_authorize_at` z `paper_intake_policy`, a nie ukrywa przycisku — żeby manager wiedział, że ścieżka istnieje, tylko jeszcze za wcześnie),
...
- **Warunki widoczności przycisku „Utwórz dokument papierowy”:**
  ...
  - ten sam próg co T1: `now() >= appointment_time + PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h (z autoryzacji powinno być już spełnione, ale UI defensywnie sprawdza).
...
| --------------- | -------------------------- | ------------------------------------------ | --------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------ |
| T1              | ADMIN/MANAGER              | Panel admin/manager → szczegóły QueueEntry | „Autoryzuj ścieżkę papierową” + reason        | WAITING, brak auth, brak doc, intake nie SUBMITTED, min. `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h po `appointment_time` | WAITING + auth istnieje, sort_at=now                         |
...
- **Nowy widok** (Django view, NIE generyczny Django admin), np. `/admin-panel/queue/<uuid>/paper-intake-authorization`:
  - **GET:** render formularza autoryzacji LUB widoku „autoryzacja istnieje” w zależności od stanu.
    - Jeśli warunki autoryzacji niespełnione (np. status nie WAITING, intake SUBMITTED, dokument istnieje, przed progiem `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h): renderuje stronę „Nie można autoryzować” z konkretnym powodem (jak DomainError z §2.1, ale w UI managera).
...
- **Brak appointment_time blokuje papier:** wpis kolejki bez `appointment_time` (np. wizyty bez harmonogramu, walk-in, awaryjne dodanie) nie może być autoryzowany papierowo, bo próg `appointment_time + PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h nie ma punktu odniesienia. To może być zaskoczeniem w klinikach, gdzie zdarzają się wizyty bez czasu — w manualu jasno: „papier wymaga zaplanowanej godziny wizyty”. Alternatywa (flag „bez ograniczenia czasowego”) wykracza poza ten plan.
...
- **Zabezpieczenie czasowe:** warunek `appointment_time + timedelta(hours=PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT)` ma być egzekwowany **w obu serwisach** (autoryzacja i utworzenie dokumentu) — duplikacja świadoma jako defense-in-depth. UI pomaga przez disable/tooltip, ale nie polega się na samym ukryciu przycisku.
...
- **Reguła czasowa:** `appointment_time` zbyt blisko `now()` (np. −1 h) → DomainError „too_early”; `appointment_time` wystarczająco w przeszłości (przy domyślnej stałej 3: np. `now() - 3h - 1min`) → OK; `appointment_time = NULL` → osobny klucz `paper_intake_requires_appointment_time` / blokada (puste appointment).
...
- **Authorize:** `POST /api/v1/queue-entries/<id>/paper-intake-authorization` happy path 201, role 403, brak `reason` 400, `reason<10` 400, `reason>500` 400, status nie WAITING 409, przed progiem `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h 409 z `available_at`, intake SUBMITTED 409, autoryzacja istnieje 409 z `authorization_id`, dokument istnieje 409, queue not found 404.
...
- **Kiedy autoryzować/wykonać:**
  ...
  - **warunek czasowy** `PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` h po `appointment_time` (ta sama stała z `apps/medical/constants.py`) dla autoryzacji ORAZ utworzenia dokumentu (egzekwowany dwukrotnie — defense-in-depth),
  - brak `appointment_time` (puste pole) blokuje obie akcje — papier wymaga zaplanowanej godziny.
- **Warunek czasowy:** autoryzacja i utworzenie dokumentu możliwe dopiero po upływie **`PAPER_INTAKE_MIN_HOURS_AFTER_APPOINTMENT` pełnych godzin** od `QueueEntry.appointment_time` (domyślnie 3). Świadome zabezpieczenie przed przypadkowym utworzeniem dokumentu przed wizytą albo w trakcie wizyty pacjenta.
